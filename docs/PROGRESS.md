...
Status: **20 etapas done, 114 vitest tests passing, typecheck + lint clean.**
...
| 20 — Sim in a Worker (increment 1) | The architecture half of the scale leap (§28): the live simulation moved **off the main thread** into a Web Worker — the `?debug` overlay reads `tick 0.0ms` on main, and the sim keeps running in a backgrounded tab (unlike a rAF loop). **Flag-gated** (`?worker`): the shipped main-thread path is verified unchanged. The worker runs its own fixed-step loop and publishes render state as a **transferable double-buffer** (`render/simFrame.ts`, pure + unit-tested — no `SharedArrayBuffer`, so no COOP/COEP); the main interpolates it through the existing RenderCar/WebGL path. A small command protocol (`simClient.ts`) keeps demand + playback live. Scope: live loop + transport controls only — the experimentation layer (mutations, A/B, optimizer, inspector, car-trace) is deferred to increment 2 (rail hidden, "engine off-thread" badge). Determinism untouched (same tick code, different host). +4 tests. |
| 19 — WebGL cars | The render half of the scale leap (§27). Phase 0 found the Canvas 2D **per-car** draw was the first wall (~6ms of ~7ms at 1500 cars), so the car layer moved to **WebGL2 instancing** — one draw call for the whole fleet — while all the tuned thermal chrome stays Canvas 2D (two stacked canvases; `drawScene` gained a `drawCars` flag). `render/glRenderer.ts` is framework-free (injected `WebGL2RenderingContext`, like `renderer.ts`): a rounded-capsule SDF shader with thermal colour + nose brightening, fed by a pure `packCarInstances` (unit-tested in Node) and the extracted `focusDimmer` spotlight. Falls back to Canvas 2D cars if WebGL2 is unavailable. Rendering is a pure read → determinism untouched. Verified live: **~7ms → ~0.7ms** CPU render at 1500 cars. +3 tests. |
| 18 — Parallel optimizer | The determinism payoff at the engineering level (§26): the sweep moved from a sequential `setTimeout` chunk to a **Web Worker pool**. Each candidate run is a pure function of `(seed, cfg, spec)`, so the action became plain data (`CandidateSpec` + `applyCandidate`) and one job is `runJob(cfg, spec, ticks) → Stats`; `sweepBaseline`/`sweepCandidate` are now thin wrappers, provably identical to the worker path (unit-tested). `sweepPool.ts` posts the structured-cloneable `ScenarioConfig` to `min(cores−1, 8)` workers, drains a job queue, and assembles the ranking (order-independent). Falls back to the chunked main thread with no `Worker` or on a worker error. Verified live under Turbopack: **8 workers**, correct leaderboard, no errors. +1 test. |
